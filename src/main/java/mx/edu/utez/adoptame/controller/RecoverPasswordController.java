...
public class RecoverPasswordController {
	...
	public String enviarContrasenaEmail(@RequestParam("correo") String correo,
		...
		try {
			...
			if (opcional != null) {
				...
				// Send message
				boolean respuestaEmail = emailServiceImpl.sendEmail(user.getCorreo(), "Cambio de contraseña",
						htmlContent);
				...
				redirectAttributes.addFlashAttribute("msg_error", "El correo no está registrado, favor de verificar");
			}

		} catch (Exception e) {
			logger.error("Error inesperado al mandar la recuperación de contraseña");
		}
	...
	public String plantillaRecuperacionContrasena(String nombreUsuario, String email, String contrasena) {
		...
		try {
			...
			logger.error("Error inesperado en la plantilla de recuperación");
		}
